...
public class SSOConnectionTest {
  ...
  public void testIdTokenInSSO() throws Throwable {
    initMock();
    SessionUtil.deleteIdTokenCache("***REMOVED***", "testuser");
    ...
    // connect url
    String url = "jdbc:snowflake://***REMOVED***";
  ...
  private void initMockHttpUtil() throws SnowflakeSQLException, IOException {
    ...
    when(HttpUtil.initHttpClient(Mockito.any(OCSPMode.class), Mockito.any(File.class)))
    ...
    String retTokenRequestIssue =
        mapper.writeValueAsString(
            new HttpUtilResponseDTO(
                true, null, new HttpUtilResponseDataTokenRequestDTO(MOCK_NEW_SESSION_TOKEN)));
    ...
    when(HttpUtil.executeGeneralRequest(
        ...
        .thenReturn(retInitialSSO, retInitialAuthentication, retTokenRequestIssue);
  ...
  private void initMockSessionUtilExternalBrowser(SFLoginInput loginInput) {
  ...
  private SFLoginInput initMockLoginInput() {
    ...
    when(loginInput.getServerUrl()).thenReturn("https://***REMOVED***/");
