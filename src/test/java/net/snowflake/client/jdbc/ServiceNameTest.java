...
public class ServiceNameTest
{
  ...
  public void testAddServiceNameToRequestHeader() throws Throwable
  {
    ...
    // login response
    when(HttpUtil.executeRequest(
        Mockito.any(HttpRequestBase.class),
        Mockito.anyInt(),
        Mockito.anyInt(),
        Mockito.any(AtomicBoolean.class))).thenReturn(
        responseLogin());
    ...
    props.setProperty(SFSessionProperty.INSECURE_MODE.getPropertyKey(), "true");
    SnowflakeConnectionV1 con =
        new SnowflakeConnectionV1("http://***REMOVED***", props);
    ...
    assertThat(((SnowflakeConnectionV1) stmt.getConnection()).getSfSession()
