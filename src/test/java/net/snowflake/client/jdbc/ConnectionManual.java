...
public class ConnectionManual {
  ...
  private String getUrl() {
    ...
    return String.format("jdbc:snowflake://%***REMOVED***:%s", account, port);
  }
  ...
  private Connection getAdminConnection() throws Throwable {
    ...
    // connect url
    String url =
        String.format("jdbc:snowflake://%***REMOVED***:%s", adminAccount, port);
  ...
  private void testSSO() throws Throwable {
    ...
    SessionUtil.deleteIdTokenCache(
        String.format("%***REMOVED***", properties.getProperty("account")),
